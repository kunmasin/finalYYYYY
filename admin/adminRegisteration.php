...
$fullname_Err = $usernames_Err  = $passwordsErr = $emailErr = $phone_numberErr = $genderErr =  $positionErr = $addressErr = " ";
$fullname = $usernames = $passwords = $email = $phone_number = $gender = $position =  $address = $image = $target_file = "";
$uploadOk = 1;

if ($_SERVER["REQUEST_METHOD"] == "POST"){
    if (empty($_POST["fullname"])){
        $fullname_Err= "FullName is required";
    }else{
    ...
    if (empty($_POST["usernames"])){
        $usernames_Err= "User Name is required";
    }else{
        $usernames= test_input($_POST["usernames"]);
        //Check if name format is valid
        if (!preg_match("/^[a-zA-Z ]*$/",$usernames)) {
        $usernames_Err = "Only letters and white space allowed";
      }
    }
...
<!--End of Php Reg Code-->
            <div class="container">
            <h1 class="text-dark text-center page-header mt-5">ADMIN REGISTERATION PAGE</h1>
            <form action="adminRegisteration.php" method="post" enctype="multipart/form-data" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
            <div class="container">
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" name="usernames" placeholder="Enter Your Username"  required>
                <br>
            </div>
            ...
            <label for="">Position</label>
                <input type="text" class="form-control" name="position" placeholder="Enter Your Position" required><br>
            </div>
