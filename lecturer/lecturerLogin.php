...
<html>
    <head>
        ...
        <body>
        <?php
    $lecturer_name =$passwords ="";
    $lecturer_nameErr = $passwordsErr = "";
    if (empty($_POST["lecturer_name"])){
        $lecturer_nameErr="lecturername is required";
    }else{
    ...
    ?>
            ...
            <h1 class="text-dark text-center page-header">LECTURER LOGIN PAGE</h1>
            <div class="container">
                <form action="lecturerLogin.php" method="post" action="<?php htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
                <div class="form-group">
                <label for="">Full Name</label>
                <input type="text" name="lecturer_name" class="form-control" placeholder="Enter Your Full Name"><br>
                </div>
            <div class="form-group">
        ...
        <h6 class="text-center"> If you have not registered already, click on the link --> <a class="text-danger" href="..\lecturer\lecturerRegisteration.php">REGISTER</a></h6>
    ...
    if (!empty($_POST)){
        ...
        $sql= "SELECT * FROM `lecturer_users` WHERE lecturer_name  LIKE '$lecturer_name' AND passwords LIKE '$passwords'";
    if(!$conn){
    die("Connection failed: " .mysqli_connect_error());
            ...
            if ($result->num_rows > 0) {
                ...
                while($row = $result->fetch_assoc()) {
                    echo "Login Successful for :<br>"."Line ". $row["id"]."<br>". " - Name: " . $row["lecturer_name"];
                    setcookie('lecturers', $row["lecturer_name"], time() + (3000), "/");
